...
const schema = z.object({
  multiDay: z.boolean(),
  allDay: z.boolean(),
  startDate: z.any().refine((v): v is Date => v instanceof Date, { message: "Pick a start date" }),
  endDate: z.any().optional(),
  startTime: z.string(),
  endTime: z.string(),
}).refine(
  (v) => !v.multiDay || v.endDate instanceof Date,
  { path: ["endDate"], message: "Pick an end date" }
).refine(
  (v) => v.allDay || v.startTime.trim().length > 0,
  { path: ["startTime"], message: "Pick a start time" }
).refine(
  (v) => v.allDay || v.endTime.trim().length > 0,
...
export default function TempUnavailabilityForm({ defaultValues, onSubmit, submitLabel = "Save" }: Props) {
      ...
      <form className="space-y-5" onSubmit={form.handleSubmit(onSubmit)}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex items-center gap-2">
              ...
              <FormLabel className="font-normal">Check if unavailable for multiple days</FormLabel>
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        <div className="space-y-2">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <DateButton value={field.value} onSelect={(d) => d && field.onChange(d)} placeholder="Pick a date" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex items-center gap-2">
              <FormControl><Checkbox checked={field.value} onCheckedChange={(v) => field.onChange(Boolean(v))} /></FormControl>
              <FormLabel className="font-normal">Check if unavailable for entire day</FormLabel>
              <FormMessage />
            </FormItem>
          )}
        />
