...
import TempUnavailabilityForm, { type TempUnavailabilityFormValues } from "@/components/forms/tempUnavailabilityForm";
...
export default function TempUnavailablilityModal({ trigger, initial, onSave }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>{trigger ?? <Button variant="outline">Add Unavailability</Button>}</DialogTrigger>
      <DialogContent className="max-w-[560px]">
        <DialogHeader><DialogTitle>Add a Temporary Unavailability</DialogTitle></DialogHeader>
