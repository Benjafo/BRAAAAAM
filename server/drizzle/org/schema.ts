import {
  pgTable, unique, uuid, text, boolean, timestamp, index, check, varchar,
  foreignKey, jsonb, uniqueIndex, integer, date, time, primaryKey, pgEnum, numeric
} from "drizzle-orm/pg-core";
import { sql } from "drizzle-orm";

export const appointmentStatus = pgEnum("appointment_status", ['Unassigned', 'Scheduled', 'Cancelled', 'Completed', 'Withdrawn']);
...
export const donationType = pgEnum("donation_type", ['Cash', 'Check', 'Envelope', 'Electronic', 'None']);
...
export const permissionAction = pgEnum("permission_action", ['read', 'create', 'update', 'delete', 'export']);
...
export const permissions = pgTable("permissions", {
  ...
  check("permissions_perm_key_format", sql`(perm_key)::text = (((resource)::text || '.'::text) || (action)::text)`),
...
export const users = pgTable("users", {
  ...
  check("users_phone_e164_check", sql`(phone IS NULL) OR (phone ~ '^\+[1-9]\d{1,14}$'::text)`),
]);
...
export const locations = pgTable("locations", {
  ...
  index("locations_alias_trgm_idx").using("gin", table.aliasName.asc().nullsLast().op("gin_trgm_ops")),
  index("locations_city_state_idx").using("btree", table.city.asc().nullsLast().op("text_ops"), table.state.asc().nullsLast().op("text_ops")),
...
export const clients = pgTable("clients", {
  ...
  index("clients_active_true").using("btree", table.id.asc().nullsLast().op("uuid_ops")).where(sql`is_active`),
  index("clients_address_idx").using("btree", table.addressLocation.asc().nullsLast().op("uuid_ops")),
  index("clients_cell_only_idx").using("btree", table.phone.asc().nullsLast().op("text_ops")).where(sql`phone_is_cell`),
  index("clients_created_at_idx").using("btree", table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
  index("clients_email_trgm_idx").using("gin", table.email.asc().nullsLast().op("gin_trgm_ops")),
  // Case-insensitive unique on email, nullable-safe:
  uniqueIndex("clients_email_uq").using("btree", sql`lower((email)::text)`).where(sql`(email IS NOT NULL)`),
  index("clients_full_name_trgm_idx").using("gin", sql`(first_name::text || ' '::text || last_name::text) gin_trgm_ops`),
  index("clients_last_name_trgm_idx").using("gin", table.lastName.asc().nullsLast().op("gin_trgm_ops")),
  foreignKey({
  ...
  check("clients_birth_year_check", sql`(birth_year IS NULL) OR ((birth_year >= 1900) AND (birth_year <= (EXTRACT(year FROM CURRENT_DATE))::integer))`),
  check("clients_birth_month_check", sql`(birth_month IS NULL) OR ((birth_month >= 1) AND (birth_month <= 12))`),
  check("clients_phone_e164_check", sql`(phone IS NULL) OR (phone ~ '^\+[1-9]\d{1,14}$'::text)`),
  check("clients_secondary_phone_e164_check", sql`(secondary_phone IS NULL) OR (secondary_phone ~ '^\+[1-9]\d{1,14}$'::text)`),
  check("phones_not_same", sql`(secondary_phone IS NULL) OR ((secondary_phone)::text <> (phone)::text)`),
]);
...
export const callLogs = pgTable("call_logs", {
  ...
  check("call_logs_phone_number_e164_check", sql`(phone_number IS NULL) OR (phone_number ~ '^\+[1-9]\d{1,14}$'::text)`),
]);
